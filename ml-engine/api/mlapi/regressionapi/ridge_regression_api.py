...
@router.post('/train/ridge-regression')
def train_ridge_regression(request: TrainingRequest):
    ...
    model = Ridge(alpha=request.alpha if request.alpha is not None else 1.0)
    model.fit(X, y)
    predictions = model.predict(X)

    artifact_dir = project_root / 'models' / 'trained'
    artifact_dir.mkdir(parents=True, exist_ok=True)
    artifact_name = f'{request.dataset_name.replace(".csv", "")}_ridge_regression.joblib'
