...
import { analyzeDataset, trainXgBoost, uploadDataset,downloadXgBoostModel } from './XgBoostApi';

function XgBoost({ onNavigate }) {
  ...
  const handleTrain = async () => {
    ...
    if(maxDepth < 1) {
      setErrorMessage('Please enter a valid maximum depth (minimum 1).');
      return;
    }

    if(learningRate <= 0 || learningRate > 1) {
      setErrorMessage('Please enter a valid learning rate (between 0 and 1).');
      return;
    }
    ...
    try {
      ...
      const data = await trainXgBoost(payload);
      setResult({
        ...data,
        artifactName: data.artifactName || 'XgBoost-model.joblib',
      });
          ...
          <div className="config-panel">
            ...
            <div className="config-row inline-field-row">
              <div className="inline-row-label">
                ...
                <span
                  ...
                  title="The number of trees in the XgBoost model. More trees can improve stability but can also increase training time. A common starting value is 100."
                  aria-label="XgBoost tree count help"
                >
                  ⓘ
                </span>
              </div>
            ...
            <div className="config-row inline-field-row">
              <div className="inline-row-label">
                ...
                <span
                  className="field-help"
                  title="The maximum depth(No of decisions) of the trees in the XgBoost model. Deeper trees can capture more complex patterns but may lead to overfitting. A common starting value is 6."
                  aria-label="XgBoost max depth help"
                >
                  ⓘ
                </span>
              </div>
            ...
            <div className="config-row inline-field-row">
              <div className="inline-row-label">
                ...
                <span
                  className="field-help"
                  title="The learning rate(each tree Contribution) of the XgBoost model. Lower values can improve stability but may require more trees. A common starting value is 0.1."
                  aria-label="XgBoost learning rate help"
                >
                  ⓘ
                </span>
              </div>
