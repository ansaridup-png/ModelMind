...
import { analyzeDataset, trainPolynomialRegression, uploadDataset,downloadPolynomialRegressionModel } from './PolynomialRegressionApi';

function PolynomialRegression({ onNavigate }) {
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      const data = await trainPolynomialRegression(payload);
      setResult({
        ...data,
        artifactName: data.artifactName || 'Polynomial-regression-model.joblib',
      });
    ...
    try {
        await downloadPolynomialRegressionModel(
        result.artifactName
        );
    } catch (error) {
  ...
  const usageSnippet = result
    ? `from sklearn.preprocessing import PolynomialFeatures\nimport joblib\n\n# Load the trained polynomial model\nmodel = joblib.load("models/trained/${result.artifactName || 'polynomial-regression-model.joblib'}")\n\n# Use feature values in the same order as the trained model\nfeature_values = [${(result.features || []).map(() => 12.5).join(', ')}]\npoly = PolynomialFeatures(degree=${degree}, include_bias=False)\nX = poly.fit_transform([feature_values])\nprediction = model.predict(X)[0]\nprint(f"Prediction: {prediction}")\n`
    : '';
