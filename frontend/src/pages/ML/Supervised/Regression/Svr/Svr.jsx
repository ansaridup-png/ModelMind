...
import { analyzeDataset, trainSvr, uploadDataset, downloadSvrModel } from './SvrApi';

const KERNEL_OPTIONS = [
  { value: 'rbf', label: 'RBF (default)' },
  { value: 'linear', label: 'Linear' },
  { value: 'poly', label: 'Polynomial' },
  { value: 'sigmoid', label: 'Sigmoid' },
];

function Svr({ onNavigate }) {
  ...
  const handleTrain = async () => {
    ...
    if (Number.isNaN(epsilon) || epsilon < 0) {
      setErrorMessage('Please enter a valid epsilon (zero or greater).');
      return;
    }

    if (!KERNEL_OPTIONS.some((option) => option.value === kernel)) {
      setErrorMessage('Please select a valid kernel.');
      return;
    }
    ...
    try {
      ...
      const data = await trainSvr(payload);
      setResult({
        ...data,
        artifactName: data.artifactName || 'svr-model.joblib',
      });
          ...
          <div className="config-panel">
            ...
            <div className="config-row inline-field-row">
              <div className="inline-row-label">
                ...
                <span
                  ...
                  title="The error allowance. Predictions that miss the true value by less than epsilon are treated as correct, so the model ignores them. Larger values give a simpler, smoother model; smaller values fit the data more tightly. A common starting value is 0.1."
                  aria-label="SVR epsilon help"
                >
                  ⓘ
                </span>
              </div>
            ...
            <div className="config-row target-row inline-field-row">
              <div className="inline-row-label">
                ...
                <span
                  className="field-help"
                  title="How the model bends the data before fitting a line. RBF handles most curved relationships, Linear suits straight-line trends, Polynomial fits curved trends of a fixed shape, and Sigmoid behaves like a neural-network activation."
                  aria-label="SVR kernel help"
                >
                  ⓘ
                </span>
              </div>

              <select value={kernel} onChange={(event) => setKernel(event.target.value)}>
