...
import { analyzeDataset, trainDecisionTree, uploadDataset } from './DecisionTreeApi';

function DecisionTree({ onNavigate }) {
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      const data = await trainDecisionTree(payload);
      setResult({
        ...data,
        artifactName: data.artifactName || 'decision-tree-model.joblib',
      });
  ...
  const handleDownloadModel = () => {
    ...
    const fileName = result.artifactName || 'decision-tree-model.joblib';
    const link = document.createElement('a');
    link.href = `http://localhost:8080/api/decisiontree/regression/download?fileName=${encodeURIComponent(fileName)}`;
